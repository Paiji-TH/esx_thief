...
RegisterNetEvent('esx_thief:animation')
AddEventHandler('esx_thief:animation', function()
	animcoger('mp_common', 'givetake1_a', 3333)
end)
...
local robbering = false

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        local ped = PlayerPedId()
		local playerPed = GetEntityCoords(ped)
		if IsControlJustPressed(1, Keys['G']) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and not IsPedInAnyVehicle(ped, true) then
			...
			local area = ESX.Game.GetPlayersInArea(playerPed, 3.0)
		...
		if robbering then
            ...
            if distance > 2.0 then
                ESX.ShowNotification(_U('you_far_away'))
				ESX.UI.Menu.CloseAll()
				robbering = false
			end
        end
...
function animcoger(dict, anim, time)
    local pid = PlayerPedId()
    RequestAnimDict(dict)
    while (not HasAnimDictLoaded(dict)) do Citizen.Wait(0) end
	  TaskPlayAnim(pid,dict,anim,100.0, 200.0, 0.3, 120, 0.2, 0, 0, 0)
	  FreezeEntityPosition(pid, true)
	  Wait(time)
	  FreezeEntityPosition(pid, false)
	  StopAnimTask(pid, dict,anim, 1.0)
end